...
activity_filtered <- activity_filtered[complete.cases(activity_filtered$sale), ]

# change date field to a date object
activity_filtered$Date <- as.Date(activity_filtered$Date , format = "%Y-%m-%d")

# create dummy var to indicate > or <= median sale
med_sale = median(activity_filtered$sale)
activity_filtered$spending = ifelse(activity_filtered$sale <= med_sale, 0, 1)
